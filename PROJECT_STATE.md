...
Current Active WIP:
- Phase 9.3a Journey Nodes Foundation is implemented in this update.
- Study Panel now includes a deferred `Journey Nodes` section identified as `ICE_JOURNEY_NODES`.
- Journey Nodes are derived, non-persisted destination records built only from current-scope Teaching Semantics, Principle Networks, Knowledge Graph, Character Interactions, Focus Lens, Scope Lens, and Study Progression.
- Node types are limited to Character, Event, Teaching, Principle, Revelation, and Fulfillment.
- Each node displays Node Name, Node Type, Why It Matters, Related Nodes, Suggested Next Nodes, Scope Boundary, source phrase, derived meaning, Provenance, Evidence Weight, Reasoning Path, confidence, and grounding.
- Related and suggested nodes are informational only and resolve only to grounded destinations already available inside the current Study Scope.
- No timeline, graph visualization, navigation control, new semantic extraction, storage key, crawling, automatic analysis, or scope mutation was added.
...
Latest pcdx update:
- Added Journey Nodes after View Lens as a deferred/collapsible section, preserving fast Study Scope-first startup.
- Controlled extension smoke confirmed 36 grounded Matthew 5 nodes with Sermon on the Mount, Beatitudes, Law Fulfillment, JESUS, and Mercy available; exact destination search is prioritized, the five-card preview reports hidden nodes honestly, no buttons render, and `ICE_JOURNEY_NODES` is not persisted.
- Targeted chapter smoke confirmed Matthew 1 destinations `Joseph receives revelation` and `Naming of JESUS`, plus Matthew 2 destinations `Flight into Egypt` and Herod.
...
- `QA_REPORTS/latest-study-panel-report.md` was regenerated by the passing session review against anchor `96196f2 pcdx: Add view lens foundation`.
- Next recommended task: push the Journey Nodes foundation commit, then run targeted Journey Node scope/provenance QA before designing journey edges or any navigation controls.
