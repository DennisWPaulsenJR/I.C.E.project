...
- Long-term Understanding Engine direction is documented in `THREAD_ARCHIVE/UNDERSTANDING_ENGINE_ROADMAP.md`: I.C.E. is not an AI assistant; external models and research systems are contributors, while I.C.E. remains responsible for evidence organization, semantic structure, provenance, certainty evaluation, relationships, competing hypotheses, presentation, and transparency.
- Future Knowledge Object and Character Profile architecture is documented in `THREAD_ARCHIVE/KNOWLEDGE_OBJECT_ARCHITECTURE.md`. It is directional architecture only and does not implement exhaustive profiles, Joseph Smith Papers ingestion, characteristic generation, multi-source confidence evaluation, crawling, storage authority, or Study View changes.
...
- Analysis Support is separate from App accuracy; it describes how directly I.C.E. can support an item from the current source and analyzed scope and is not a judgment on scriptural truth.
- User-facing support labels are Strong, Supported, and Limited instead of High, Moderate, and Limited confidence.
...
- Controlled Matthew 5 smoke for Phase 9.5a confirmed 5 support panels, `ICE_CONFIDENCE_ANALYSIS` layer identity, all required fields, and a working source reference inside the panel.
- Next recommended task: user presentation review of Analysis Support wording, especially Scene Context tier clarity.
