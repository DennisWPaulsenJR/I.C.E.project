...
Current Active WIP:
- Phase 9.3c Journey Hubs Foundation is implemented in this update.
- Study Panel now includes a deferred `Journey Hubs` section identified as `ICE_JOURNEY_HUBS`.
- Journey Hubs are derived, non-persisted convergence records built from current-scope Journey Nodes, Journey Paths, Knowledge Graph, Principle Networks, Character Interactions, and Teaching Semantics.
- Ordinary node hubs require grounded path degree plus resolved neighboring Journey Nodes; aggregate Teaching, Character, Fulfillment, and Revelation hubs require multiple grounded destinations plus path or semantic-record support.
- Connected Nodes always resolve to existing current-scope Journey Nodes, and Connected Paths reuse existing current-scope Journey Paths.
- Hub types are limited to Character, Event, Teaching, Principle, Fulfillment, and Revelation.
- Each hub displays Hub Name, Hub Type, Connected Nodes, Connected Paths, Why Central, Scope Boundary, Provenance, Evidence Weight, Reasoning Path, source phrase, derived meaning, confidence, and grounding.
- No visualization, traversal controls, automatic navigation, new semantic extraction, persisted Journey Hubs key, crawling, automatic analysis, queue execution change, or scope mutation was added.
...
Latest pcdx update:
- Added Journey Hubs after Journey Paths as a deferred/collapsible section, preserving fast Study Scope-first startup.
- Controlled extension smoke produced five major hubs per fixture and confirmed grounded examples: `JESUS CHRIST` in Matthew 1, `Fulfillment` in Matthew 2, and `Sermon on the Mount` plus `JESUS as Teacher` in Matthew 5.
- Smoke confirmed exact requested fields, deferred loading, no action buttons, no `ICE_JOURNEY_HUBS` storage entry, and no Study Panel page errors.
- The `JESUS CHRIST` hub requires exact current-scope Knowledge Graph identity evidence and explicitly preserves narrative `JESUS` as a distinct display form.
- No `background.js` or storage behavior changed.
- Validation passed: `node --check study.js`, `git diff --check`, `npm.cmd run qa:matthew-pages`, and `npm.cmd run review:matthew-session`.
- `QA_REPORTS/latest-study-panel-report.md` was regenerated by the passing session review against anchor `6422803 pcdx: Add journey paths foundation`.
- Next recommended task: targeted Journey Hub scope/provenance QA or display refinement; visualization, traversal controls, automatic navigation, and automatic analysis remain deferred.
