...
const TEST_URL = "https://www.churchofjesuschrist.org/study/scriptures/nt/matt/5?lang=eng";
const EXTENSION_ROOT = path.resolve(__dirname, "..");
const OUTPUT_DIR = path.join(EXTENSION_ROOT, "qa-output");
const OUTPUT_FILE = path.join(OUTPUT_DIR, "latest-matthew5-qa-bundle.json");
const STORAGE_KEYS = [
  "ICE_LATEST_CAPTURE",
  "ICE_ACTIVE_ADAPTER",
...
function hasTeachingSemantic(data, discourseType, predicate = () => true) {
...
function evaluateFailures(data) {
  ...
  if (!hasTeachingSemantic(data, "teaching_block", (item) =>
    ...
    (item.groupEntities || []).some((group) => /Group Entity/i.test(group.entityType || "") && /Class III -> Class III/i.test(group.currentGroundedClassRange || ""))
  )) failures.push("Expected Matthew 5 teaching block with JESUS speaker, disciples/multitudes audience, and Class III group entity range.");

  if (!hasTeachingSemantic(data, "blessing", (item) => /poor in spirit/i.test(item.blessing || "") && /kingdom of heaven/i.test(item.promise || ""))) failures.push("Expected poor in spirit blessing discourse record.");
  if (!hasTeachingSemantic(data, "blessing", (item) => /meek/i.test(item.blessing || "") && /inherit the earth/i.test(item.promise || ""))) failures.push("Expected meek blessing discourse record.");
  if (!hasTeachingSemantic(data, "blessing", (item) => /merciful/i.test(item.blessing || "") && /obtain mercy/i.test(item.promise || ""))) failures.push("Expected merciful blessing discourse record.");
...
async function runBackgroundAnalysis(worker) {
  return await worker.evaluate(async () => {
    if (typeof runFullAnalysisPipeline === "function") {
      return await runFullAnalysisPipeline("qa:matthew5");
    }
...
async function main() {
  ...
  try {
    ...
    console.log(bundle.pass ? "PASS: Matthew 5 extension QA" : "FAIL: Matthew 5 extension QA");
    ...
    console.error("FAIL: Matthew 5 extension QA");
