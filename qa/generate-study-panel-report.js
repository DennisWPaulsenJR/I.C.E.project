...
}function depthLensLines(samples, limit = 8) {
  return asArray(samples.depthLens).slice(0, limit).map((item) => `${clean(item.currentDepth || "Depth")} | Expansion: ${clean(item.expansionLevel || "semantic expansion")} | Layers: ${asArray(item.enabledSemanticLayers).slice(0, 5).map(clean).join(", ") || "enabled layers awaiting records"} | Evidence: ${clean(item.evidenceWeight || "Derived Semantic Evidence")}`);
...
function reportFromBundle(bundle) {
  ...
  const depthLens = asArray(samples.depthLens);
  const semanticQuestions = asArray(samples.semanticQuestions);
  const trustVerification = asArray(samples.trustVerification);
  ...
  return [
    ...
    `Depth Lens: ${counts.depthLens || depthLens.length || 0}`,
    ...
    ...list([
      ...
      ...depthLens.slice(0, 4).map((item) => provenanceLine(item.provenance || "I.C.E. Depth Lens", `${item.currentDepth || "Depth"} depth`, "Depth Lens", "ICE_DEPTH_LENS")),
    ...
    ...list([
      ...
      ...depthLens.slice(0, 4).map((item) => evidenceWeightLine(item.evidenceWeight || "Derived Semantic Evidence", "depth classification derived from enabled semantic layer families", item.sourceGrounding || item.derivedMeaning, asArray(item.enabledSemanticLayers).length)),
    ...
    "",
    "## Depth Lens",
    ...list(depthLensLines(samples), "no depth lens records available"),
    "",
    "## Trust & Verification",
    ...
    ...list([
      ...
      (counts.depthLens || depthLens.length) ? "Depth Lens: Strict/Grounded/Elaborate display record found" : "Depth Lens: Awaiting semantic layer depth summary",
    ...
    ...list([
      ...
      ...depthLens.slice(0, 5).map((item) => `${item.currentDepth || "Depth"} depth | ${item.expansionLevel || "semantic expansion"} | App accuracy: ${item.confidence || "probable"}`),
    ...
    `depthLens: ${counts.depthLens || depthLens.length || 0}`,
